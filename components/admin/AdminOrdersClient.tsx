...
import { MessageCircle, Search, Download, RotateCcw, Trash2 } from "lucide-react";
...
export function AdminOrdersClient({ initialOrders }: { initialOrders: Order[] }) {
  ...
  const filtered = orders.filter((o) => {
    ...
    const matchesSearch =
      ...
      (o.user.client?.companyName ?? "").toLowerCase().includes(q);
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({
          ...
          description: `Pedido foi marcado como ${statusNames[newStatus] || newStatus}.`,
        });
      } else {
        toast({
          type: "error",
  ...
  const deleteOrder = async (order: Order) => {
    if (
      !confirm(
        `Excluir permanentemente o pedido ${order.orderNumber}? Esta ação não pode ser desfeita.`
      )
    )
      return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({
          ...
          description: `Pedido ${order.orderNumber} foi removido com sucesso.`,
        });
      } else {
        const body = await res.json();
        toast({ type: "error", title: body.error || "Erro ao excluir pedido" });
      }
    } catch {
      toast({ type: "error", title: "Erro de conexão", description: "Não foi possível excluir o pedido." });
    } finally {
      setDeletingId(null);
    }
  };
                      ...
                      <div className="flex items-center justify-end gap-1.5 flex-wrap">
                        ...
                        <button
                          onClick={() => {
                            ...
                            openWhatsApp(
                              phone,
                              `Olá! Aqui é do Atacadão Embalagens sobre o pedido #${order.orderNumber}.`
                            );
                          }}
                        ...
                        {/* Botão excluir — somente para pedidos terminais (DELIVERED ou CANCELLED) */}
                        {isTerminal && (
                          <button
                            onClick={() => deleteOrder(order)}
                            disabled={deletingId === order.id}
                            className="p-1.5 rounded-lg text-surface-400 hover:text-red-400 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-all disabled:opacity-50"
