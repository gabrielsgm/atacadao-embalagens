...
export function CartDrawer() {
  ...
  // Fetch profile completeness when the cart opens
  useEffect(() => {
    if (!state.isOpen || session?.user?.role === "ADMIN") return;
    fetch("/api/clients/me")
      .then((r) => r.json())
      .then((data) => setProfileStatus(data))
      .catch(() => {});
  }, [state.isOpen, session?.user?.role]);

  if (!state.isOpen) return null;

  const profileIncomplete = profileStatus !== null && !profileStatus.profileComplete;

  const handleFinalize = async () => {
    if (state.items.length === 0) return;

    // Client-side guard: show friendly message before even hitting the API
    if (profileIncomplete) {
      toast({
        type: "error",
        title: "Cadastro incompleto",
        description: "Complete seu perfil para realizar pedidos.",
      });
      return;
    }
    ...
    try {
      ...
      if (res.status === 403) {
        ...
        toast({
          ...
          description: body.description || "Complete seu perfil para realizar pedidos.",
        });
        closeCart();
          ...
          <div className="border-t border-surface-700 p-4 space-y-4">
            ...
            {profileIncomplete && (
              <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/25">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-amber-300">Cadastro incompleto</p>
                  <p className="text-xs text-amber-400/80 mt-0.5">
                    Complete seu perfil para finalizar o pedido.
                  </p>
                </div>
                <Link
                  href="/minha-conta/perfil"
                  onClick={closeCart}
                  className="shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold transition-colors"
                  id="cart-complete-profile-link"
                >
            ...
            >
              {profileIncomplete ? "Cadastro necessário" : "Finalizar e enviar pedido"}
            </Button>
