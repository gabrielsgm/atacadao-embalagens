...
export default async function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pending clients urgent alert */}
      {pendingClients > 0 && (
        <Link
          href="/admin/clientes"
          className="flex items-center gap-4 p-4 rounded-2xl bg-yellow-500/10 border border-yellow-500/30 hover:border-yellow-500/60 hover:bg-yellow-500/15 transition-all group"
        >
          <div className="relative shrink-0">
            <div className="h-12 w-12 rounded-2xl bg-yellow-500/20 border border-yellow-500/30 flex items-center justify-center">
              <Clock className="h-6 w-6 text-yellow-400" />
            </div>
            <span className="absolute -top-1.5 -right-1.5 flex h-5 w-5">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-yellow-400 opacity-40" />
              <span className="relative inline-flex rounded-full h-5 w-5 bg-yellow-500 text-black text-[10px] font-black items-center justify-center">
                {pendingClients}
              </span>
            </span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-yellow-300">
              {pendingClients} cadastro{pendingClients > 1 ? "s" : ""} aguardando aprovação
            </p>
            <p className="text-xs text-yellow-400/70 mt-0.5">
              Analise e aprove os novos clientes para liberar o acesso à plataforma.
            </p>
          </div>
          <ArrowRight className="h-5 w-5 text-yellow-400/60 group-hover:text-yellow-300 group-hover:translate-x-1 transition-all shrink-0" />
        </Link>
      )}
