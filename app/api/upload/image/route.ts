...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Se Cloudinary estiver configurado, tenta fazer upload la
    if (process.env.CLOUDINARY_CLOUD_NAME && process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET) {
      try {
        const { url } = await uploadImage(buffer, "atacado-embalagens/products", publicId);
        return NextResponse.json({ url });
      } catch (cloudErr) {
        console.warn("[CLOUDINARY_FALLBACK] Falha no Cloudinary, usando Data URL:", cloudErr);
      }
    }

    // Fallback automatico: Salva como Data URL Base64 direto no banco
